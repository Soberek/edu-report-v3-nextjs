...
describe("FileUploader", () => {
  ...
  it("should call onFileUpload when file is selected", () => {
    ...
    const fileInput = screen.getByLabelText("Wybierz plik Excel do wczytania") as HTMLInputElement;
  ...
  it("should clear file input value after selecting a file", () => {
    renderWithTheme(<FileUploader {...defaultProps} />);

    const fileInput = screen.getByLabelText("Wybierz plik Excel do wczytania") as HTMLInputElement;
    const file = new File(["test"], "another.xlsx", { type: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" });
    let currentValue = "C:/fakepath/another.xlsx";
  ...
  it("should clear input value when reset is triggered", async () => {
    const user = userEvent.setup();
    renderWithTheme(<FileUploader {...defaultProps} fileName="test.xlsx" />);

    const fileInput = screen.getByLabelText("Wybierz plik Excel do wczytania") as HTMLInputElement;
    let currentValue = "C:/fakepath/test.xlsx";
