...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!apiKey) {
      return NextResponse.json(
        { error: 'PostImages API key not configured' },
        { status: 500 }
      );
    }
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        { error: `PostImages API error: ${response.status} ${response.statusText}` },
    ...
    if (responseText.startsWith('<!DOCTYPE') || responseText.startsWith('<html')) {
      ...
      return NextResponse.json(
        { error: 'PostImages API returned HTML error page' },
        { status: 400 }
      );
    }
    ...
    if (responseText.startsWith('<?xml')) {
      ...
      const thumbMatch = responseText.match(/<thumbnail>(.*?)<\/thumbnail>/i) || responseText.match(/<thumb>(.*?)<\/thumb>/i) || responseText.match(/thumb="([^"]+)"/i);
      ...
      if (errorMatch) {
        const errorText = errorMatch[1].replace(/&lt;br&gt;/g, ' ').replace(/&lt;/g, '<').replace(/&gt;/g, '>');
        console.error('PostImages XML error:', errorText);
        return NextResponse.json(
          { error: `PostImages API error: ${errorText}` },
          { status: 400 }
        );
      }
      ...
      return NextResponse.json(
        { error: 'PostImages XML response format not recognized' },
        { status: 400 }
      );
    }
    ...
    try {
      ...
      return NextResponse.json(
        { error: 'Invalid JSON response from PostImages' },
        { status: 400 }
      );
    }
    ...
    if (data.success && data.image) {
      ...
      return NextResponse.json(
        { error: data.message || 'PostImages upload failed' },
        { status: 400 }
      );
    }
  } catch (error) {
