...
const StatisticsPage: React.FC = () => {
  ...
  const { state } = useBudgetMeter();

  // Calculate monthly data for charts
  const monthlyData = React.useMemo(() => {
    if (!state.rawData.length || !state.aggregatedData) {
      return [];
    }

    const selectedMonthNumbers = state.selectedMonths.filter((m) => m.selected).map((m) => m.monthNumber);
  ...
  }, [state.rawData, state.selectedMonths, state.aggregatedData]);
    ...
    <Container maxWidth="xl" sx={{ py: 4 }}>
      ...
      <Box sx={{ mb: 6, textAlign: "center" }}>
        ...
        <Typography variant="h6" color="text.secondary" sx={{ maxWidth: 600, mx: "auto" }}>
          Wizualizacja liczby działań i odbiorców według miesięcy
        </Typography>
      </Box>
      ...
      {!hasData ? (
        ...
        >
          ...
          <Typography variant="body1">Wczytaj plik Excel i wybierz miesiące, aby zobaczyć statystyki</Typography>
        </Box>
      ) : (
